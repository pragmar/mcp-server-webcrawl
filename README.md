...
Extras provide a means of producing token-efficient HTTP content responses. Markdown produces roughly 1/3 the bytes of the source HTML, snippets are generally 500 or so bytes per result, and xpath can be as specific or broad as you choose. The more focused your requests, the more results you can fit into your LLM session. The idea, of course, is that the LLM takes care of this for you. If you notice your LLM client developing an affinity to requesting the "content" field (full HTML), a nudge in LLM chat to budget tokens using the extras feature should be all that is needed.
